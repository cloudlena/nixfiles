...
let
  ...
  # Mango has no dispatcher for "move the window to a new tag", so the first
  # empty tag of the focused window's monitor is looked up over the IPC socket
  # and handed to the regular tag dispatcher
  tagEmpty = pkgs.writeShellScript "mango-tag-empty" ''
    set -u

    monitor="$(mmsg get focusing-client \
      | ${pkgs.jq}/bin/jq --raw-output '.monitor // empty')"
    [ -n "$monitor" ] || exit 0

    tag="$(mmsg get tags "$monitor" \
      | ${pkgs.jq}/bin/jq --raw-output \
        'first(.tags[] | select(.client_count == 0) | .index) // empty')"
    [ -n "$tag" ] || exit 0
  ...
  screenshot = pkgs.writeShellScript "screenshot" ''
    ...
    file="${screenshotDir}/$(date +'%F-%H%M%S').png"
    ${pkgs.grim}/bin/grim -g "$geometry" - \
      | tee "$file" \
      | ${pkgs.wl-clipboard}/bin/wl-copy --type image/png
    ${pkgs.libnotify}/bin/notify-send -a screenshot -i "$file" \
      "Screenshot" "Copied to clipboard and saved to ${screenshotDir}"
  '';
  ...
  xdg.configFile = {
    "mango/config.conf".text = # ini
      ''
        ...
        scratchpadcolor=0x${theme.colors.primary}ff
        ...
        # Window manager
        bind=SUPER,Tab,switcher,all_prev
        ...
        bind=SUPER+SHIFT,n,spawn,${tagEmpty}
