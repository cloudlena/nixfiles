...

{
  ...
  # Background image. wbg has no daemon/client split: the process holds the
  # layer-shell surface for the whole session, so the image is a startup
  # argument and survives restarts of the unit on its own.
  systemd.user.services.wbg = {
    ...
    Service = {
      ExecStart = "${pkgs.wbg}/bin/wbg --stretch ${config.xdg.dataHome}/wallpapers/bespinian.png";
      Restart = "always";
      RestartSec = 1;
    };
    Install = {
